...
class ThumbnailGenerator:
    ...
    def create_thumbnail_from_analysis(
        ...
        try:
            ...
            3. **パワーワードの適切な使用**:
               - 「日経平均」「米国株」「NVIDIA」等は、**そのニュースの主役である場合のみ**使用してください。
               - 昨晩・今日と「半導体」が続いているなら、あえて「為替」や「個別決算」「新NISA戦略」など、別のフックを探してください。

            4. **知名度の低い専門用語のみを翻訳・補足**:
            ...
            6. **具体的な「報酬」と「数字」**:
               - 「+〇〇万円」「利回り〇％」「資産爆増」「お宝株」など、視聴者が得られる具体的メリットを盛り込んでください。
