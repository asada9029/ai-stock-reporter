...
# 台本作成の鉄則（コンセプト：初心者フレンドリー＆ロジカル）
    ...
    3. 【読み上げ対策（重要）】：LIXIL（リクシル）やNVIDIA（エヌビディア）など、アルファベットの企業名や英単語は、音声合成で一文字ずつ（エル・アイ...等）読まれないよう、必ず読み上げ台本（text）の中では自然なカタカナ表記（例：リクシル、エヌビディア、ナスダック）に変換してください。ただし、画面表示用（on_screen_text）はアルファベットのままで構いません。
