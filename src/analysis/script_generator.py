...
class ScriptGenerator:
    ...
    def _character_prompt_block(self) -> str:
        ...
        return f"""
# キャラクター設定（番組の顔）
- 株野みのり（解説 / speaker=`minori`）: {minori.get('speaking_style') or '敬語。やさしいが芯が硬い。'}
- 相場カリン（相棒 / speaker=`karin`）: {karin.get('speaking_style') or 'タメ口寄りの後輩。旬に飛びつく。'}
※「やさしいのにちょっと尖ってる」トーン。まじめニュース番組にしない。ほのぼのたとえを適宜入れる。
- **名乗り**: speech_text / dialogue では「ホスト」「パートナー」「MC」等の制作内部用語は禁止。「株野みのりです」「みのりです」「カリンです」など視聴者に自然な言い方のみ。
...
- 口調の個性は保つが、**声に感情演技を乗せない**。叫び・囁き・「！」「っ」の連発は禁止。VOICEVOXは常にノーマル読み上げなので、聞き取りやすい通常のセリフにする。
"""
...
- on_screen_text は「■」「└」形式は禁止。図解シーンは最大2行の補足。**ニュース／文字中心シーンは3〜5行**で密度を確保（1行のみは禁止）。**各行は日本語必須**（全文英語禁止。英字はティッカー・指数略称のみ可）。
...
- 【日本語必須】`text` / `on_screen_text` / `dialogue.text` は日本語。Hello / Welcome などの英語挨拶・英語全文は禁止。
...
- 【読み上げと表示の分離（最重要・日本語必須）】:
  - `text` / `on_screen_text` / `dialogue.text` は **必ず日本語**（字幕にそのまま出る）。全文英語の台本は禁止。
  - 英字を残してよいのはティッカー・指数略称のみ（例: NVDA, S&P500）。文章・挨拶・説明は日本語。
...
解説役は「株野（かぶの）みのり」。相棒は「相場カリン」。掛け合いは opening / 旬ニュース / closing で使う。
...
- duration: 秒数
- text: ナレーション本文（**日本語必須**。字幕にも使用。全文英語は禁止）
- speech_text: 読み上げ専用（textと同内容だが英字はカタカナ読み）
- on_screen_text: 文字列の配列（画面表示用・**日本語必須**。画像がある場合は最大4行まで。以下の2項目1セットで構成）
    1. "■ [事実・見出し]"
    2. "  └ [考察・注意点]"
    事実・見出しというのは、ニュースやデータの客観的な要約（例：「S&P500 終値3,856」「エヌビディア 営業益20%増」）
    考察・注意点というのは、事実・概要に対する分析や投資家が注意すべき点（例：「米金利上昇が重石」「円安による上振れに注目」）
...
- dialogue: 任意。掛け合い時は配列。各要素に speaker / text / speech_text（text も日本語必須）
...
# 台本作成の鉄則（コンセプト：徹底的な初心者目線＆ロジカル）
    ...
    3. 【読み上げと表示の分離（重要・日本語必須）】：`text` / `on_screen_text` / `dialogue.text` は **必ず日本語**（字幕に出る）。全文英語は禁止。英字はティッカー・指数略称（NVDA, S&P500 等）のみ可。`speech_text` にだけカタカナ読み（エヌビディア、エスアンドピー500 等）を書く。
    ...
    8. 【感情（キャラ表情のみ。声には乗せない）】
        - `emotion`: 立ち絵切替用。中立的な説明・数値の読み上げ・つなぎは **normal** でよい。
        ...
        - **禁止**: 感情を speech_text / dialogue に演技として書かない。叫び・囁き・過剰な感嘆・「っ」連発は禁止。読み上げは常に聞き取りやすい通常口調。
    8b. 【emotion_timeline（表情切替。声は変えない）】
        ...
        - timeline は立ち絵だけが変わる。読み上げの抑揚・話速・スタイルは変えない。
...
- text: ナレーション本文（**日本語必須**。字幕にも使用。全文英語は禁止）
- speech_text: 読み上げ専用（textと同内容だが英字はカタカナ読み）
- on_screen_text: 文字列の配列（画面表示用・**日本語必須**。最大4行まで。以下の2項目1セットで構成）
    1. "■ [事実・見出し]"
    2. "  └ [考察・注意点]"
...
"""
        ...
        while attempt < max_retries:
            ...
            try:
                ...
                # --- 表示テキストの日本語正規化（英語テロップ事故の保険） ---
                from src.analysis.japanese_text import (
                    count_english_display_issues,
                    normalize_scenes_japanese_display,
                )

                n_ja_fixed = normalize_scenes_japanese_display(scenes)
                if n_ja_fixed:
                    print(f"[JA] 英語寄りの表示テキストを speech_text 等へ正規化: {n_ja_fixed} シーン")
                en_count, en_issues = count_english_display_issues(scenes)
                if en_count > 0 and not is_shorts:
                    print(f"[WARN] 日本語表示チェック未達: {en_count} 件 — {'; '.join(en_issues[:4])}")
                    last_errs.extend(en_issues[:8])
                    if attempt >= max_retries:
                        # 正規化後も残る場合は警告のみで続行（完全ブロックしない）
                        print("[WARN] 英語表示が残っていますが、正規化済みのため続行します")
                    else:
                        quality_retry_appendix += (
                            "\n# 【前回出力の表示言語エラー — 厳守】\n"
                            "- `text` / `on_screen_text` / `dialogue.text` は必ず日本語で書いてください。\n"
                            "- 英語の挨拶・英語全文台本は禁止です（Hello / Welcome / Minori: 等）。\n"
                            "- 英字はティッカー・指数略称（NVDA, S&P500 等）のみ許可します。\n"
                        )
                        continue
