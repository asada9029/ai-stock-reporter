...
3. 【us_news_highlights】: **重要：以下の順番でニュースを紹介してください。**
    ...
    - **【ニュース画像必須】**: 紹介する各ニュースで `attention_news[i].visual_image_path` が存在する場合、必ず `target_files: ["<そのパス>"]` と `image_type: "chart"` を設定してください（OG画像も chart として表示可）。パスが無い場合のみ `image_type: "character_only"` または `bg_only`。
...
3. 【news_highlights】: **重要：以下の順番でニュースを紹介してください。**
    ...
    - **【ニュース画像必須】**: 紹介する各ニュースで `attention_news[i].visual_image_path` が存在する場合、必ず `target_files: ["<そのパス>"]` と `image_type: "chart"` を設定してください。パスが無い場合のみテキスト中心の構成にしてください。
