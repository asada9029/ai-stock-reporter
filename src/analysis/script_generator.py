...
- on_screen_text は「■」「└」形式は禁止。1シーンあたり最大3行まで。各行は短いラベル（目安：全角12〜16文字以内）。
- 良い例: "S&P500 -1.2%", "半導体 +4.2%", "注目: エヌビディア決算", "日経 3万8500円台"
- 悪い例: 音声で言っている長文をそのまま画面に載せること、長い箇条書き。
- 画像があるシーンは on_screen_text は1行推奨（数値＋キーワードのみ）。
- 【opening 上書き】:
...
- 【強調（重要）】視線誘導のため、各シーンで「強調したい語」を `emphasis` に2〜4個指定してください。
  - 形式: `emphasis: [{{\"text\":\"+5.2%\",\"style\":\"up\"}}, {{\"text\":\"下方修正\",\"style\":\"warn\"}}]`
  - style は必ず次のいずれか: up, down, warn, key, neutral
  - text は `speech_text` の重要語（数字、会社名、材料）から選ぶ。長文は避ける（目安: 2〜8文字程度）。
...
- 【注目ニュースの画面（重要）】OG画像は必須ではありません（内容とズレることがあるため）。ニュースの「何の話か」を画面で必ず明確にしてください。
  ...
  - 追加キー（任意だが推奨）: `ticker`, `company_name`（関連銘柄と同じでOK）, `emphasis`
"""
